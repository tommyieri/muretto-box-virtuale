...
def episodi(P, win, zona, chiusura, K):
    """([(esito, L)], rese, censurati) con esito in {conv, nonconv}.

    Il GIRO L in cui l'episodio comincia viaggia insieme all'esito: serve a dividere
    gli episodi fra le due meta' della gara (ai_lab/confronto/PREREG_sorpassi_intragara.md,
    STRADA v2(a) di data/SORPASSO_NOTA.txt). NON cambia quali episodi esistono ne' come
    si risolvono: il cancello I0 verifica che i gate G0-G3 diano gli stessi identici
    numeri di prima della modifica."""
...
def calcola(setting, cache):
    ...
    for cid, stag, P, win in cache:
        ...
        d[0] += sum(1 for e, _ in eps if e == 'conv'); d[1] += len(eps)
        d[2] += rese; d[3] += cens
    return per_cid
