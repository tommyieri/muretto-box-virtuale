...
// --- coefficienti MISURATI DAL FONDO (data/modello_*.json), non inventati qui
const TRAFFICO_FONDO = { a: 0.74781, lam: 0.5 };      // = kappa*a, lam del modello traffico 2026
const RHO = { SOFT: 0.0541, MEDIUM: 0.0441, HARD: 0.0398 };   // modello degrado 2026
...
function valuta(c, opzioni) {
  const { state, pace, comp } = inputs(c);
  const extra = { ...opzioni };
  if (extra.degrado === 'FONDO') extra.degrado = degradoDa(c, comp);
  let paceUso = pace;
