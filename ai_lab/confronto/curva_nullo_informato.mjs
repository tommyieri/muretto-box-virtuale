#!/usr/bin/env node
// curva_nullo_informato.mjs — LA FISICA AGGIUNGE QUALCOSA OLTRE L'ARITMETICA DELLA SOSTA?
//
//     node ai_lab/confronto/curva_nullo_informato.mjs [--json]
//
// Esegue PREREG_nullo_informato.md. SOLA LETTURA.
//
// La curva precedente (curva_orizzonte.mjs) confrontava il motore col nullo «l'ordine al
// congelamento non cambia». Quel nullo NON SA che il soggetto e' entrato ai box — e la
// sosta cade dentro la finestra in tutti e 274 i casi. Quindi buona parte del vantaggio
// misurava il sapere che una sosta e' avvenuta, non la fisica.
//
// Qui il nullo sa della sosta: e' l'ordine al congelamento con la perdita ai box applicata
// al soggetto, e NIENT'ALTRO. La differenza che resta e' cio' che il motore modella in
// piu' — degrado, carburante, evoluzione dei rivali, neutralizzazione.
//
// NON E' UNA RI-MISURA DI F1: il KPI firmato dice «batte il non-fare-niente», e il nullo
// letterale e' quello vecchio. Questa e' una domanda PIU' DIFFICILE, dichiarata come tale
// nella prereg prima di guardare qualsiasi numero.
...
function motoreAOrizzonte(caso, h) {
  ...
  if (!ordine.includes(caso.pilota)) return null;
  // LA PERDITA CHE IL KERNEL HA DAVVERO APPLICATO, non una ricalcolata altrove (prereg §3):
  // `scenario.perdita` e' un OGGETTO con targhetta e provenienza, e la prima scrittura lo
  // ha passato tale e quale al nullo — che ha sommato un oggetto a un cum e ha prodotto
  // NaN, cioe' zero casi utilizzabili a ogni orizzonte. Il numero che il kernel paga sta
  // in `pits[pilota][0].perdita`: e' quello, e non un suo cugino.
  const sosta = sc?.pits?.[caso.pilota]?.[0]?.perdita;
  return { ordine, perdita: Number.isFinite(sosta) ? sosta : null };
}

/**
 * IL NULLO INFORMATO: l'ordine al congelamento con la perdita ai box addebitata al
 * soggetto. Sa che si e' fermato e quanto gli e' costato; non sa nient'altro.
 */
function nulloInformato(gSim, Lf, soggetto, perdita) {
  const cum = {};
  for (const [drv, celle] of gSim.perPilota) {
    const c = celle.get(Lf);
    if (c && Number.isFinite(c.cum_time)) cum[drv] = c.cum_time + (drv === soggetto ? perdita : 0);
  }
...
for (const h of GRIGLIA) {
  ...
  for (const c of casi()) {
    ...
    const mm = motoreAOrizzonte(c, h);
    if (!mm || !Number.isFinite(mm.perdita)) { scartati += 1; continue; }
    const motore = mm.ordine;
    const nullo = nulloInformato(gSim, c.freezeLap, c.pilota, mm.perdita);
...
console.log('');
console.log('══ IL NULLO CHE CONOSCE LA SOSTA — PREREG_nullo_informato.md ════════════════════');
...
console.log('');
console.log('   MOTORE CONTRO IL NULLO INFORMATO (che sa della sosta)');
...
if (JSON_OUT) {
  const doc = {
    _targhetta: {
      cosa_e: 'La fisica del motore aggiunge qualcosa oltre l\'aritmetica della sosta? Nullo = ordine al congelamento + pit-loss al soggetto.',
      prereg: 'ai_lab/confronto/PREREG_nullo_informato.md',
  ...
  const dove = path.join(RADICE, 'ai_lab', 'confronto', 'ESITO_curva_nullo_informato.json');
